...
class MultiHeadAttentionSpatial(nn.Module):
    def __init__(
        ...
        if self.spatial_attn_fusion in ['mul', 'bias', 'add']:
            ...
            self.view_spatial_loc_fc = nn.Linear(d_model, self.spatial_n_head)
    ...
    def forward(self, q, k, v, pairwise_locs, key_padding_mask=None, txt_embeds=None, view_spatial_attention=None):
        residual = q
        ...
        if self.spatial_attn_fusion in ['mul', 'bias', 'add']:
            # print(view_spatial_attention.shape)
            loc_attn = self.pairwise_loc_fc(pairwise_locs) #  pairwise_locs
            # loc_attn = loc_attn + self.view_spatial_loc_fc(view_spatial_attention)
            ...
            loc_attn = torch.einsum('hbld,bltd->hblt', spatial_weights, pairwise_locs) + spatial_bias    # pairwise_locs
            loc_attn = torch.sigmoid(loc_attn)
        # print(self.spatial_attn_fusion, loc_attn.shape, view_spatial_attention.shape)
